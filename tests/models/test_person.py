...
def test_person_retrieve_sets_the_id(successful_client: AbstractClient):
    person = Person.retrieve(id="foo", client=successful_client)
    assert person.id == "foo"
    assert isinstance(person, Person)


def test_person_retrieve_error_is_raised(failed_client: AbstractClient):
    with pytest.raises(RequestFailedError):
        Person.retrieve("foo", client=failed_client)


def test_person_retrieve_sets_properties(successful_client: AbstractClient):
